...
public:
	...
	PlayerSAO(ServerEnvironment *env_, RemotePlayer *player_, maskedu16 peer_id_, bool is_singleplayer);
	...
	*/
	...
	std::string getClientInitializationData(u16 protocol_version, u16 ID);
	void getStaticData(std::string *result) const;
	void step(float dtime, bool send_recommended);
	void setBasePosition(const v3f &position, bool dont_send = false);
	...
	*/
	...
	void disconnected(bool goingto_otherserv);
	void disconnected() {
		disconnected(false);
	}
	...
	std::string GetInitData(u16 proto);
	void setServerState(const bool st) { _slave_server = st; }
private:
