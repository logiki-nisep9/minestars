...
struct NetworkPacketPlusPeer {
	NetworkPacketPlusPeer() = default;
	NetworkPacket *pkt;
	session_t peer_id;
};
...
struct ServerPlayingSound
{
	...
	std::unordered_set<u16> clients_int16; // same as the variable up this line but with uint16_t
};
...
public:
	...
	*/
	...
	void OnlyDeleteSAO(session_t peer_id, u16 ID_);

	//dunno
	void sendDetachedInventory(Inventory *inventory, const std::string &name, maskedu16 PLAYER_ID); //x
	void SendPlayerInventoryFormspec(maskedu16 pid); //x
	void SendPlayerPrivileges(maskedu16 pid); //x
	void SendDeathscreen(maskedu16 pid, bool set_camera_point_target, v3f camera_point_target); //x
	void SendPlayerFov(maskedu16 PID); //x
	void SendMovePlayer(maskedu16 PID); //x
	void SendPlayerSpeed(maskedu16 PID, const v3f &added_vel); //x
	void SendShowFormspecMessage(maskedu16 player_id, const std::string &formspec, const std::string &formname); //x
	void SendMovement(maskedu16 pid, u16 protocol_version); //x
	void SendHUDChange(maskedu16 pid, u32 id, HudElementStat stat, void *value); //x
	void SendHUDAdd(maskedu16 pid, u32 id, HudElement *form); //x
	void SendHUDSetFlags(maskedu16 pid, u32 flags, u32 mask); //x
	void SendHUDSetParam(maskedu16 pid, u16 param, const std::string &value); //x
	void SendHUDRemove(maskedu16 pid, u32 id); //x
	void SendBreath(maskedu16 pid, u16 breath); //x
	void SendSetSky(maskedu16 pid, const SkyboxParams &params); //x
	void SendEyeOffset(maskedu16 pid, v3f first, v3f third); //x
	void SendOverrideDayNightRatio(maskedu16 pid, bool do_override, float ratio); //x
	void SendCloudParams(maskedu16 pid, const CloudParams &params); //x
	void SendSetSun(maskedu16 pid, const SunParams &params); //x
	void DeleteClientU16(u16 pid, ClientDeletionReason reason); //x
	void SendSetMoon(maskedu16 pid, const MoonParams &params); //x
	void SendSetStars(maskedu16 pid, const StarParams &params); //x
	bool getClientInfo(maskedu16 pid, ClientInfo &ret); //x
	void QueuePlayerToInitialize(session_t p);
	void InitializePlayer(session_t pid);
	void QueueProcessData(NetworkPacket pkt);
	u16 GetRandomIDforPlayer(); //Useful to generate multiple numbers
	bool ExistsID(u16 ID);
	void SendMinimapModes(maskedu16 pid, std::vector<MinimapMode> &modes, size_t wanted_mode);
	void SendDisconnectToPlayer(u16 ID);
	PlayerSAO* InitClientByMainServer(u16 ID_, ClientDataHelper *client);
	...
	u8 getSerializationVersion(session_t peer_id);
	void QueueOnJoinPlayer(PlayerSAO *sao, const char *name);
	void RunScriptsOnLeave(session_t PID);
	void SendRemoveObjectsToClient(u16 pid);
	...
	void SendChatMessage(maskedu16 PID, const ChatMessage &message);

	std::mutex DEFINITIONS_EXECUTION;

	std::unordered_map<u16, u16> Jumper; //Only for slave

	void SendActiveObjectMessages(maskedu16 pid, const std::string &datas, bool reliable = true);
	...
	//Some adds
	void SendSpawnParticle(maskedu16 playerid, u16 protocol_version, const ParticleParameters &p);
	...
	*/
	...
	void sendDetachedInventories(maskedu16 player_id, bool incremental);
	...
	bool SendBlock(maskedu16 pid, const v3s16 &blockpos);
...
private:
	...
	void SendPlayerHP(maskedu16 pid);
	void SendHP(maskedu16 PID, u16 hp);

	void SendLocalPlayerAnimations(maskedu16 player, v2s32 animation_frames[4], f32 animation_speed);
	...
	void SendPlayerFormspecPrepend(maskedu16 pid);
	...
	*/
	...
	void SendBlockNoLock(maskedu16 playerid, MapBlock *block, u8 ver, u16 net_proto_version);
	...
	void SendAddParticleSpawner(maskedu16 playerid, u16 protocol_version, const ParticleSpawnerParameters &p, u16 attached_id, u32 id);

	void SendDeleteParticleSpawner(session_t peer_id, u32 id);
	void SendDeleteParticleSpawner(maskedu16 playerid, u32 id); //x
	...
	*/
	...
	void DiePlayer(maskedu16 pid, const PlayerHPChangeReason &reason);
	void RespawnPlayer(session_t peer_id);
	void RespawnPlayer(maskedu16 PID);
	...
	PlayerSAO *getPlayerSAO(maskedu16 PID);
	...
	*/
	...
	PlayerSAO *emergePlayer(const char *name, maskedu16 pid, u16 proto_version);
	...
	*/
	...
	std::unordered_map<u16, ClientDataHelper*> ClientDataTable;

	//Some tables that might help to make multithreaded context
	std::mutex QueuedPacketsMTX;
	std::deque<NetworkPacket> QueuedPackets; //This is needed to handle packets for the main server (Specified as main proxy players at ONLY.)
	std::unordered_map<session_t, u16> SessionsToU16; //List which are need the PlayerIDs
	std::unordered_map<session_t, bool> SessionsStatus; //List if the player are playing in a slave server
	std::unordered_map<session_t, u16> PlayerInEnvironmentToServer; //hbving a seizure
	std::unordered_map<u16, bool> ActivePlayersU16; //Check
	std::unordered_map<session_t, bool> SeenSlaveServers; //Useful when checking slave-sent packets to don't conflict with client packets
	std::unordered_map<session_t, u16> SeenSlaveServersIDS;
