...
public:
	...
	void        clearHud();
	u32         getMaxHuds() { return m_max_huds; }
	...
	float SsumZ = 0.0f;
	float SsumY = 0.0f;
	//f32 lastYaw = 0.0f; //no used
	f32 lastPitch = 0.0;
	u32 LastKeyPressed = 0;
	bool multicraftplayer = false;
	bool sneakpressed = false;
	bool lastsneakpressed = false;
	bool btn_dig   = false;
	bool btn_place = false;
	
	//patterns which will be used to detect same entry or not
	bool pt_w = false;
	bool pt_s = false;
	bool pt_a = false;
	//bool pt_cl = false;
	...
	u16 get_peer_id() { return player_id; }

	//Server Proxy (11/02/25)
	SlaveServer *OnServer = nullptr; //Handles which server the player stand
	u16 player_id = 0; //Stands for recognize players on the slave servers
	bool to_other_server = false; //Stands for PlayerSAO
