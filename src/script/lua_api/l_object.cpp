...
int ObjectRef::l_send_mapblock(lua_State *L)
{
	...
	if (srv->ServersNetworkObject->AreSlave) {
		...
		u16 PID = player->player_id;
		r = getServer(L)->SendBlock(maskedu16(PID), pos);
	}
...
int ObjectRef::l_add_velocity(lua_State *L)
{
	...
	if (!srv->ServersNetworkObject->AreSlave) {
		...
		if (sao->getType() == ACTIVEOBJECT_TYPE_LUAENTITY) {
			...
			PlayerSAO *playersao = dynamic_cast<PlayerSAO*>(sao);
			playersao->setMaxSpeedOverride(vel);
			maskedu16 n = maskedu16(playersao->getPlayerID());
			srv->SendPlayerSpeed(n, vel);
		}
	}
...
int ObjectRef::l_set_fov(lua_State *L)
{
	...
	if (!srv->ServersNetworkObject->AreSlave) {
		...
		srv->SendPlayerFov(maskedu16(player->player_id));
	}
	return 0;
}
...
int ObjectRef::l_set_minimap_modes(lua_State *L)
{
	...
	if (!srv->ServersNetworkObject->AreSlave)
		...
		srv->SendMinimapModes(maskedu16(player->player_id), modes, selected_mode);
	return 0;
}
