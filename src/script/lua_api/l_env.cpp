...
#else
	//if (legacy) {
	//	warningstream << "Doing Legacy ABM [Deprecated, please change!] {mod_name='" + modorig + "', label='" + name + "'}" << std::endl;
	//} else {
	//	actionstream << "Doing ABM [First step] {mod_name='" + modorig + "', label='" + name + "'}" << std::endl;
	//}
#endif
}
		...
		u32 active_object_count, u32 active_object_count_wider)
{
	...
	if (is_legacy) {
		...
		scriptIface->queueABM(std::string(funcsrz, func_length), p, n, active_object_count, active_object_count_wider);
	}
...
int ModApiEnvMod::l_get_connected_players(lua_State *L)
{
	...
	for (RemotePlayer *player : env->getPlayers()) {
		if (!getServer(L)->ServersNetworkObject->AreSlave)
...
int ModApiEnvMod::l_get_player_by_name(lua_State *L)
{
	...
	if (!player || !getServer(L)->ServersNetworkObject->AreSlave)
