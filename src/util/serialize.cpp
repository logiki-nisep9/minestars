...
//Note: The VeryLongStrings functions needs to be used only for media transmissions...

#define VERY_LONG_STRING_MAX_LEN 5.9029581e+20

std::string serializeString64(const std::string &plain)
{
	std::string s;
	char buf[8];

	s.reserve(8 + plain.size());

	writeU64((u8*)&buf[0], plain.size());
	s.append(buf, 8);
	s.append(plain);
	return s;
}

std::string deSerializeString64(std::istream &is)
{
	std::string s;
	char buf[8];
	...
	u64 s_size = readU64((u8 *)buf);
	if (s_size == 0)
		return s;

	// We don't really want a remote attacker to force us to allocate 4GB...
	if (s_size > UINT64_MAX/2) {
		throw SerializationError("deSerializeLongString: "
			"string too long: " + itos(s_size) + " bytes");
	}
