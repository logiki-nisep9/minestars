...
nether.register_portal("xp_portal", {
	...
	title = S("XP Death Portal"),
	book_of_portals_pagetext = S("This is a portal to your death place, build this like a bowl 3x3 as the image specify, then use a mese shard to power on it!"),
	is_within_realm = function(pos) -- return true if pos is inside the Nether
		return false
	end,
	find_realm_anchorPos = function(surface_anchorPos, player_name)
		local xp = MineStars.XP.GetXP(player_name)
		if xp >= 120 then
			return MineStars.Player.GetLastPlayerDeathPos(player_name)
		else
			return Player(player_name):get_pos()
		end
	end,
	dynamic_portal_entrance = function(player, portal_definition, local_anchorPos)
		local xp = MineStars.XP.GetXP(player)
                if xp >= 120 then
                        return MineStars.Player.GetLastPlayerDeathPos(player)
                else
                        return Player(player):get_pos()
                end
	end,
	only_teleport = true,
	on_player_teleported = function(portalDef, player, oldPos, newPos)
		--print("GOT!")
		if not vector.equals(oldPos, newPos) then
			MineStars.XP.RemoveXP(player, 100)
		end
	end
})
