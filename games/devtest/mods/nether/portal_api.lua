...
local function ensure_remote_portal_then_teleport(playerName, portal_definition, local_anchorPos, local_orientation, destination_wormholePos)
	...
	--[[print(dump({
		playerName,
		portal_definition,
		core.pos_to_string(local_anchorPos),
		core.pos_to_string(destination_wormholePos)
	}))--]]
	...
	local enable_sound_return_d = 555;

	if portal_definition.only_teleport then
		if portal_definition.dynamic_portal_entrance then
			destination_wormholePos = portal_definition.dynamic_portal_entrance(player, portal_definition, local_anchorPos)
		end
		if portal_definition.on_player_teleported then
			enable_sound_return_d = portal_definition.on_player_teleported(portal_definition, player, local_anchorPos, destination_wormholePos)
		end
		player:set_pos(destination_wormholePos)
		return
	end

	if enable_sound_return_d ~= 555 then
		if enable_sound_return_d == nil then
			enable_sound = true;
		else
			enable_sound = enable_sound_return_d;
		end
	else
		enable_sound = true;
	end
