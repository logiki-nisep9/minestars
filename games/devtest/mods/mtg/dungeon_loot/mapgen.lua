...
local function populate_chest(pos, rand, dungeontype)
	dungeontype = dungeontype or "normal"
	...
	if not inv then core.log("error", "Something nasty is happening"); return; end
	local listsz = inv:get_size("main")
	--//core.log("error", dump{listsz, #items})
	--assert(listsz >= #items)
...
default.populate_chest = populate_chest;
...
minetest.register_on_generated(function(minp, maxp, blockseed)
	...
	for _, room in ipairs(rooms) do
		...
		if minetest.get_node(chestpos).name == "air" then
			...
			minetest.set_node(chestpos, {name = "default:chest", param2 = facedir})
			populate_chest(chestpos, PcgRandom(noise3d_integer(noise, chestpos)), room.type)
			
			----
			-- Due to engine changes, the functions as set_node, swap_node and remove_node are queued in the ServerEnvironment for being executed, because
			-- of a.b.m.e (Active Block Modifier Environment) and Async Environment.
			-- So the callbacks might make a reasonable solution to this problem, or core.after right after placed a node.
			-- ~Logiki
			----
			
			--math.abs(math.floor(noise:get_3d(pos) * 0x7fffffff))
			
			--(core.get_perlin(10115, 4, 0.5, 1), pos)
			--[[print(dump(room))
			core.set_node(chestpos, {name = "default:chest", param2 = facedir}, function(pos, agrs)
				core.log("error", "Here!"); print(populate_chest, noise3d_integer, core, default, dump(args))
				default.populate_chest(pos, PcgRandom(math.abs(math.floor(core.get_perlin(10115, 4, 0.5, 1):get_3d(pos) * 0x7fffffff))), agrs[1])
				--error()
			end, room.type)--]]
		end
	end
