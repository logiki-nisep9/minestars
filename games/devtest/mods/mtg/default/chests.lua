...
local function randomposs(pos)
	local x_ = math.random(-0.5, 0.5);
	local z_ = -x_;
	local x = math.random(pos.x, pos.x+x_);
	local z = math.random(pos.z, pos.z+z_);
...
function default.chest.register_chest(prefixed_name, d)
	...
	if def.protected then
		...
		def.can_dig = function(pos,player)
			local meta = minetest.get_meta(pos);
			local inv = meta:get_inventory();
			local bool = default.can_interact_with_node(player, pos);
			if bool then
				local drops = core.get_meta(pos):get_inventory():get_list("main");
				for _, item in pairs(drops) do
					core.add_item(randomposs(pos), item) --###ASSUMING pos AS v3f (preconverted = randomposs) & item as UserData::ItemStack (738)
				end
				return true;
			else
				return false;
