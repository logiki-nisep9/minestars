    ...
    if not debug then
        minetest.register_lbm({
            ...
            action = function(pos, node)
                ...
                if not inv then core.log("error", "Error at getting inventory, something suspicious is happening") end
                for i = 1, slots do
                    if rand:next(0,100) <= slot_spawn_chance then
                        ...
                        local _ = inv and inv:set_stack("main", i, stack) or function() core.log("error", "ERROR!") end
                    end
                end
            end,
        })
