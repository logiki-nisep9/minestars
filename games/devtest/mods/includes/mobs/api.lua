...
function mobs:register_mob(name, def)
	...
	minetest.register_entity(":" .. name, setmetatable({
		...
		on_die = def.on_die,
		on_death = def.on_death, -- engine function for entity death --def.on_death
		on_flop = def.on_flop,
...
--print("--- not enough space to spawn", name)
	...
	if map_load == true then
		...
		minetest.register_lbm({
			...
			action = spawn_action--function(pos, node)
		...
		minetest.register_abm({
			...
			catch_up = false,
			legacy_abm = true, --i surrender making too much things
			min_y = min_height, max_y = max_height,

			action = spawn_action--function(pos, node, active_object_count, active_object_count_wider)
