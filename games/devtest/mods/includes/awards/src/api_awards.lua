...
function awards.unlock(name, award)
	...
	if awards.show_mode == "chat" then
		...
		--core.chat_send_all(core.colorize("lightgreen", arrow..MineStars.Translator("@1 Has unlocked the award '@2'!", name, core.colorize("green", title))))
		MineStars.Chat.SendAll("notif:good", MineStars.Translator("@1 Has unlocked the award '@2'!", name, core.colorize("#ACFCAC", title)), name)
		MineStars.SO_Effects.DoEffect(Player(name), "award")
	end
end
