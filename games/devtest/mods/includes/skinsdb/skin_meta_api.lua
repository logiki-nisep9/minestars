...
function skin_class:get_head()
	if self._head then
		return self._head
	end
	local player_skin = "("..(self:get_texture() or "character.png")..")"
	return "[combine:16x32:-4,-8=" .. player_skin .. "^[mask:skindb_mask_head.png"
end
...
function skin_class:apply_skin_to_player(player)
	...
	player:set_properties({
		...
		textures = {
			v10_texture.."^[mask:onlybody.png",
			v18_texture,
			armor_texture.."^[mask:onlybodyarmor.png",
			wielditem_texture,
		},
	})
	
	core.after(0.2, MineStars.UpdateHead, player)
