	...
	if self.textures[name] then
		if player_api then
			player_api.set_textures(player, {
				...
				self.textures[name].armor.."^[mask:onlybodyarmor.png", --keep the helmet skin for other part
				self.textures[name].wielditem,
			})
		else
			local text = player:get_properties().textures
			text[3] = self.textures[name].armor
...
armor.get_textures = function(self, player)
...
core.register_on_joinplayer(function(player)
	armor.PhysicsOf[player:get_player_name()] = {
		speed = 1,
		jump = 1,
		gravity = 1,
	}
...
armor.set_player_armor = function(self, player)
	...
	elseif use_pova_mod then
		...
		if player_physics_locked == nil or player_physics_locked == 0 then
			...
			--local physics_ = player:get_physics_override()
			--for stat, val in pairs(physics) do
			--	physics_[stat] = val
			--end
			--armor.PhysicsOf[Name(player)] = physics_
			if MineStars then
				MineStars.Physics.DefaultPhysics[player:get_player_name()] = physics
...
function GetPhysics(player)
	local name = player:get_player_name()
	return armor.PhysicsOf[name]
end
