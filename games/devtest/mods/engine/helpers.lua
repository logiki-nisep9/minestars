...
function AlwaysReturnPositiveInteger(int)
	if int < 0 then
		return 0
	else
		return int
...
function IsWalkable(nodename)
	return core.registered_items[nodename] and ((core.registered_items[nodename] == true) or (core.registered_items[nodename] == nil))
end

function GetBoolFromInt(INT, FALLBACK)
	return (INT < 1 and true) or (FALLBACK or false)
end

function ReturnNonUpperLimit(numb1, numb2)
	if numb1 > numb2 then
		return numb2
	else
		return numb1
